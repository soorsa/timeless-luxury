...
async function getWatchProduct(id: string) {
  const watch = products.find((i) => i.slug === id);
  return watch;
}
...
export async function generateMetadata({
  ...
  const watch = await getWatchProduct(slug);
  ...
  return {
    title: `${watch.name} · Timeless Luxury`,
    description:
...
export default async function ProductDetailPage({ params }: PageProps) {
  const { id: slug } = await params;
  const product = await getWatchProduct(slug);
  if (!product) {
    return (
